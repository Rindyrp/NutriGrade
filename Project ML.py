...
class NutrientAnalyzer:
    ...
    def extract_nutrients_from_text(self, text: str) -> Dict[str, float]:
        ...
        # Pattern untuk menangkap angka dengan satuan, termasuk desimal dan variasi spasi
        patterns = {
            'sugar': r'(?:' + '|'.join(self.nutrient_aliases['sugar']) + r')\s*:?\s*(\d+\.?\d*|\.\d+)\s*(?:g|gram|grams)',
            'sodium': r'(?:' + '|'.join(self.nutrient_aliases['sodium']) + r')\s*:?\s*(\d+\.?\d*|\.\d+)\s*(?:mg|milligram|milligrams)',
            'saturated_fat': r'(?:' + '|'.join(self.nutrient_aliases['saturated_fat']) + r')\s*:?\s*(\d+\.?\d*|\.\d+)\s*(?:g|gram|grams)'
        }
        
        for nutrient, pattern in patterns.items():
            matches = re.findall(pattern, text, re.IGNORECASE)
            if matches:
                # Ambil nilai tertinggi jika ada multiple matches, pastikan konversi ke float
                try:
...
def preprocess_image(image: Image.Image) -> Image.Image:
    ...
    # Noise reduction dengan bilateral filter untuk menjaga tepi
    ...
    # Adaptive thresholding untuk binarisasi yang lebih baik
    binary = cv2.adaptiveThreshold(
        enhanced, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
        cv2.THRESH_BINARY, 15, 5
    )
    
    # Dilation untuk memperkuat teks
...
def detect_and_crop_nutrition_label(image: Image.Image) -> Image.Image:
    ...
    # Edge detection dengan parameter yang disesuaikan
    ...
    for contour in contours:
        ...
        if area > 15000:  # Area minimum disesuaikan
            x, y, w, h = cv2.boundingRect(contour)
            aspect_ratio = w / h
            
            # Label nutrisi biasanya berbentuk persegi panjang
            if 0.2 < aspect_ratio < 4.0 and w > 250 and h > 250:
...
def perform_ocr(image: Image.Image) -> str:
    """Melakukan OCR menggunakan EasyOCR untuk ekstraksi teks yang akurat"""
    try:
        # Inisialisasi EasyOCR reader
        reader = easyocr.Reader(['en', 'id'], gpu=False)  # Dukungan bahasa Inggris dan Indonesia
